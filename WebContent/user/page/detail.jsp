...
<head lang="en">
    ...
    <link href="../js/jquery-3.1.1.min.js" rel="javascript">
    <style>
		#img_zly{
			width: 25px;
		}
...
<div id="nav_detail">
    <%
        List<TbBook> bookList = (List<TbBook>) request.getAttribute("book");
        TbBook book = bookList.get(0);
        
        //
        boolean exit_collect=false;
        int exit_collect_Integer=0;
        Integer integer = Integer.valueOf( session.getAttribute("user_id").toString());
        int user_id_zly = integer;
        exit_collect=new check_user().User_collect_book_exist(user_id_zly, book.getIsbn());
        if(exit_collect==true)
        	exit_collect_Integer=1;
        else
        	exit_collect_Integer=0; 
    ...
    <div class="right_detail lf">
       ...
       <%-- <h3>(印度，Gopi，Subramanian，萨伯拉曼尼安 著；方延风，刘丹 译)</h3>--%>
        ...
        <p class="accountChose">
            ...
            <input type="text" value="1" class="number" id="buy-num" name="buy-num">
        ...
        <div class="shops">
            ...
            <%--加入购物车 request.getParameter("buy-num") --%>
            <% 
            String change_zly_01 =null;
            System.out.println(exit_collect_Integer); 
            if(exit_collect_Integer==1)
           		change_zly_01="product_detail_img62.png";
            
            else
            	change_zly_01="uncollect01.png";
            System.out.println(change_zly_01);
            %>
            <a href="#" class="shop lf" id="add_cart"><img src="../img/product_detail/product_detail_img7.png" alt=""/>加入购物车</a>
            <a href="#" class="collection lf" id="collect"><span>收藏</span></a><b><img id="img_zly" src='../img/product_detail/<%=change_zly_01%>' alt=""/></b>
        </div>
    </div>
</div>
<!--为你推荐-->
...
/*    $("#add_cart").click(function (e) {
	...
	$("#buy_now").on("click",function(){
        ...
        var url = "Password_And_ChangeAddress?method=Buy_now&isbn=<%=book.getIsbn()%>&count="+buyCount;
    ...
    $("#add_cart").on("click",function(){
        buyCount = $("#buy-num").val();
        <%-- var url = "<%=basePath%>CartAddBook?method=cartAddBook&isbn=<%=book.getIsbn()%>&count="+buyCount;
        ...
        $.ajax({
	        type:"POST",
	        url:"<%=basePath%>CartAddBook?method=cartAddBook&isbn=<%=book.getIsbn()%>&count="+buyCount,
    ...
    var $num=0;
    <%-- $num<%=exit_collect_Integer%>;
    <%System.out.println(exit_collect_Integer);%> --%>
    $("#collect").on("click",function(){
    	
    	var url = "Book_collect?method=Book_collect&isbn=<%=book.getIsbn()%>&exits=<%=exit_collect_Integer%>";
    	 ...
    	 */
    	if($num%2==0){
    		$("#img_zly").attr({src:"../img/product_detail/uncollect01.png"});
    	}
    	if($num%2==1){
    		$("#img_zly").attr({src:"../img/product_detail/product_detail_img62.png"});
    	}
    });
